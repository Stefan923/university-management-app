...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    <S extends User> Page<S> findAllByFirstNameContainingIgnoreCaseAndLastNameContainingIgnoreCase(
            @RequestParam("firstName") String firstName,
            @RequestParam("lastName")  String lastName,
            Pageable pageable);

    @RestResource(path = "byRole", rel = "findUserByRole")
    <S extends User> Page<S> findAllByRoleId(
            @RequestParam("roleId") long roleId,
            Pageable pageable);

    @RestResource(path = "byUsername", rel = "findUserByUsername")
    <S extends User> Page<S> findAllByAccountUsernameContainingIgnoreCase(
            @RequestParam("username") String username,
            Pageable pageable);

    @RestResource(path = "byEmail", rel = "findUserByEmail")
    <S extends User> Page<S> findAllByAccountEmailContainingIgnoreCase(
